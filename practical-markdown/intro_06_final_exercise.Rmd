...
gap <- gap %>% mutate(group = case_when(
...
> Step 5: Convert the group variable to a factor and cross tabulate it with continent to check the reassignation worked

```{r}
gap <- gap |> mutate(group = as.factor(group))

gap |> select(group, continent) |> tbl_summary(by = continent)
```
...
```{r}
summary(gap$infant_mortality)
summary(gap$dollars.pp.pd)
...
lm(inf.surv*100 ~ log.dpd, gap.sum) |> tbl_regression()
...
gap |> 
  ...
  lm((1- (infant_mortality/1000))*100 ~ log10(dollars.pp.pd), data = _ ) |> 
  tbl_regression()
