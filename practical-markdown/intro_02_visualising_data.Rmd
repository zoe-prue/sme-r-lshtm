---
title: "Session 2 - Visualising Data - 2026"
...
You should have at this point already made sure you've set up R according to the Getting Started file and have already run your first commands in R.
...
There is an ongoing 'debate' between whether one should use the tidyverse for data analysis or stick with the functions that come natively with R. SME and ASME use a number of packages in the name of laziness and pedagogy - why do excess work if someone has done that work previously? Packages greatly extend the functionality of R, though the downfall is that sometimes they change (and your old code may have to be updated - though this is a rare occurrence!). Using the tidyverse will also help you to think about the analysis, not about the specifics of how R works. As you become a more advanced user, you will need to understand more about Base R - and then you'll come to your own conclusion!

Load the libraries below, either by pressing the green triangle at the top right corner of the chunk, or pressing Ctrl-Enter with your cursor in each line. Do not highlight the whole line!

```{r}
#--- Load libraries
# Make sure you have these packages installed first!
# If not, run: install.packages(c("psych", "tidyverse"))
library(psych)
library(tidyverse)
```

To start investigating our question, we need a dataset. Most data comes to you in a csv (comma separated values) file from Excel - we will talk about other formats later. This dataset, sdg.csv, should've come with this Rmd file - make sure that it is located in a folder entitled 'data' in the same parent folder as this Rmd file, else the code will not work. You can also save it in the same folder, in which case you will need to remove the '/data' part of the filepath.

We use a simple function to read in data: read_csv().

```{r}
#--- Read in the data
sdg <- read_csv("./data/sdg.csv")

# Note: Absolute paths like "E:/LSHTM/Teaching/r4epi/sdg.csv" are bad practice
# because they won't work on other computers. Always use relative paths with "./"
...
> EXERCISE: Get detailed summary statistics for maternal mortality, both overall and stratified by LMIC status.
...
This may seem like a minor difference, but an important part of coding is legibility - by always ensuring the data is on the left, you can help someeone unfamiliar with your code to figure out what each component does. You may not see the magrittr dollar sign pipe in SME or ASME, but you may wish to use it. We'll use pipes more extensively later.
...
Below are some plotting exercises. You may wish to attempt these now and see where you get to, and return to them after you have gained some confidence in R. Remember that it is completely OK to Google, use Claude, or use StackExchange - part of R is figuring out what to search for to get an answer!
...
Once you've created a plot you're happy with, you'll want to save it for use in reports or presentations. R makes this easy with the `ggsave()` function.

### Basic Usage

The simplest way to save your last plot:
...
### Customizing Your Saved Plot

You can control the size, resolution, and format:

```{r, eval = FALSE}
# Create a plot
ggplot(sdg, aes(x = gini, y = tb, color = reg)) +
  geom_point() +
  theme_bw() +
  labs(title = "Income Inequality vs TB Incidence")

# Save with custom settings
ggsave("tb_gini_plot.png",
       width = 8,           # Width in inches
       height = 6,          # Height in inches
       dpi = 300)           # Resolution (300 dpi is publication quality)
```

### Different File Formats

You can save in various formats by changing the file extension:

```{r, eval = FALSE}
ggsave("plot.png")   # PNG (good for presentations, web)
ggsave("plot.pdf")   # PDF (good for publications, scales well)
ggsave("plot.jpg")   # JPEG (smaller file size)
ggsave("plot.svg")   # SVG (vector format, editable)
```

### Saving a Specific Plot

If you've created multiple plots, you can save a specific one by assigning it to an object:

```{r, eval = FALSE}
# Assign plot to an object
my_plot <- ggplot(sdg, aes(x = urb, y = tb)) +
  geom_point() +
  theme_bw()

# Save the specific plot
ggsave("urban_tb.png", plot = my_plot, width = 6, height = 4, dpi = 300)
```
