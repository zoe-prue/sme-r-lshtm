...
You'll notice that lm() takes a 'data' argument, but it is not the leftmost argument. To specify where the left argument should go, we use the underscore. Remember that tbl_regression() presents the coefficients nicely -- if you want the raw data, you need to pipe to summary() and tell R what you're piping in with a fullstop.
...
> Step 1: Install the dslabs package. Load its library. Ensure you also have loaded the gtsummary and tidyverse packages.
...
> Step 5: Convert the group variable to a factor and cross tabulate it with continent to check the reassignation worked (hint: tbl_summary() with by = continent)
...
The function below is written in LaTeX -- a language for displaying equations. If you hover over it, the formula will display.
